...
#c.Session.username = 'peakbreaker'
...
def post_save(model, os_path, contents_manager):
    #print('Jupyter savehool registered, attempting post save routine!')
    _, ext = os.path.splitext(os_path)
    if model['type'] != 'notebook' or ext != '.ipynb':
        return # only do this for notebooks
    # Post-save hook for converting notebooks to .py scripts
    d, fname = os.path.split(os_path)
    print(f'filetype is {model["type"]}, fname is {fname}')
    if not timestamped_file(fname):
        print('CONVERT: notebook to pythonfile and html')
        check_call(['jupyter', 'nbconvert', '--to', 'script', fname], cwd=d)
        check_call(['jupyter', 'nbconvert', '--to', 'html', fname], cwd=d)
    else:
        print('SKIPPING: convert of notebook to pythonfile and html')
